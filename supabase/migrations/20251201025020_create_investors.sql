...
-- Central database of investors with all matching criteria
-- ================================================
...
CREATE TABLE IF NOT EXISTS public.investors (
  ...
  -- Portfolio & Activity
  portfolio_companies JSONB DEFAULT '[]'::jsonb, -- Array of {name, website, industry, stage, description}
...
CREATE INDEX IF NOT EXISTS idx_investors_check_size ON public.investors(typical_check_size_min, typical_check_size_max);
...
-- Only admins can manage investors (check if user_roles table exists)
DO $$
BEGIN
  ...
  ELSE
    -- Fallback: service role only (for seeding)
