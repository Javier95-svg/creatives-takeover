-- Upgrade Daniela Hagg to Professional tier with Pro-level credit allocation.
-- Requested account:
-- - daniela.a.hagg@gmail.com (0ce1f981-4fea-45ed-94d1-a2b540b17bee)
--
-- This migration is idempotent and safe to run multiple times.

DO $$
DECLARE
  target_user_id UUID := '0ce1f981-4fea-45ed-94d1-a2b540b17bee';
  target_email TEXT := 'daniela.a.hagg@gmail.com';
...
BEGIN
  resolved_email := lower(target_email);

  -- Resolve user by UID first, then by email as fallback.
  SELECT u.id INTO resolved_user_id
  FROM auth.users u
  WHERE u.id = target_user_id OR lower(u.email) = resolved_email
  ORDER BY CASE WHEN u.id = target_user_id THEN 0 ELSE 1 END
  LIMIT 1;

  IF resolved_user_id IS NULL THEN
    RAISE EXCEPTION '[pro-upgrade] user not found for email=% uid=%', resolved_email, target_user_id;
  END IF;

  IF resolved_user_id <> target_user_id THEN
    RAISE WARNING '[pro-upgrade] uid mismatch for email=% expected_uid=% resolved_uid=%',
      resolved_email, target_user_id, resolved_user_id;
  END IF;
