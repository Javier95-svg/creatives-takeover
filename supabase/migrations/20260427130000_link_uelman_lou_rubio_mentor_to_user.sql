DO $$
DECLARE
  target_user_id UUID := 'e6248878-3f18-4866-9e2e-7ad39f0339de';
  target_email TEXT := 'dendysan@gmail.com';
  target_mentor_name TEXT := 'Uelman Lou Rubio';
...
BEGIN
  ...
  WHERE lower(name) = lower(target_mentor_name)
    OR (
      lower(name) LIKE '%uelman%'
      AND lower(name) LIKE '%rubio%'
    );
