-- Link Selma Fetic's mentor profile to their messaging account so the Message CTA opens DMs

DO $$
DECLARE
  target_user_id UUID := '37bf2612-8b43-406e-a4c1-8213859437f6';
  target_email   TEXT := 'selma.f.ze@gmail.com';
  target_name    TEXT := 'Selma Fetic';
...
BEGIN
  ...
  WHERE
    ...
    OR (
      lower(name) LIKE '%selma%'
      AND lower(name) LIKE '%fetic%'
    );
