-- Reduce dashboard task bell noise to one aggregate reminder per user per day.
...
BEGIN
  ...
  IF EXISTS (
    SELECT 1
    FROM cron.job
    WHERE jobname = 'task-overdue-reminders-daily'
  ) THEN
    PERFORM cron.unschedule('task-overdue-reminders-daily');
  END IF;
END;
$$;

SELECT cron.schedule(
  'task-overdue-reminders-daily',
  '0 14 * * *',
