DO $$
DECLARE
  target_user_id UUID := 'e8ddb66e-142b-4d88-9d4f-7ce3cf18ce14';
  target_email TEXT := 'albert.hovhannisian@gmail.com';
  target_mentor_name TEXT := 'Albert Hovhannisyan';
...
BEGIN
  ...
  WHERE lower(name) = lower(target_mentor_name)
    OR (
      lower(name) LIKE '%albert%'
      AND (
        lower(name) LIKE '%hovhannisyan%'
        OR lower(name) LIKE '%hovhannisian%'
      )
    );
