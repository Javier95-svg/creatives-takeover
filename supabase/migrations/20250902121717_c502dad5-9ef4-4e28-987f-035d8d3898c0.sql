-- Enable realtime for community posts and comments (only if not already added)
ALTER TABLE community_posts REPLICA IDENTITY FULL;
ALTER TABLE post_comments REPLICA IDENTITY FULL;
ALTER TABLE profiles REPLICA IDENTITY FULL;

-- Check and add tables to realtime publication only if not already present
DO $$
BEGIN
    -- Add community_posts if not already present
    IF NOT EXISTS (
    ...
    -- Add post_comments if not already present
    IF NOT EXISTS (
        SELECT 1 FROM pg_publication_tables 
