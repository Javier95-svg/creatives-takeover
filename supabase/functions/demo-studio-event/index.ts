  ...
  try {
    ...
    const { data: demo, error: demoError } = demoId
      ? await admin.from("demo_studio_demos")
        .select("id,project_id,status,demo_studio_projects!inner(id,validation_context_id)")
    ...
    const { data: publicProject } = !demo && projectId
      ? await admin.from("demo_studio_projects").select("id,validation_context_id,launch_published").eq("id", projectId).maybeSingle()
      : { data: null };
