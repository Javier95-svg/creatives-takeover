...
test('Build page removes fabricated social proof', () => {
  for (const unsupportedClaim of [
    'Who&rsquo;s Next?', '1,000+ founders', '4.8/5',
    'Featured in the founder feeds you actually read', 'Jordan Rivera', 'Maya Karlsson',
    'Sam Lin', 'Priya Nair', 'Tomas Vega', 'Aisha Bello',
  ]) {
...
test('Build page presents evidence-backed positioning with honest product proof', () => {
  for (const capability of [
    'ICP + PMF context', 'Evidence-scoped MVP', 'Real code you control',
    'Your evidence, already here', 'Don&rsquo;t start from a', 'ICP Builder',
    'Demo Studio', 'PMF Lab', 'MVP Builder brief', 'Build from my evidence',
...
test('Build page presents focused scope, ownership, and shipping', () => {
  for (const capability of [
    'Focus is a feature', 'prove the business.', 'The evidence-backed scope contract',
    '≤3 essential features', 'No more than 3 essential features',
...
test('Build page keeps the agreed narrative order and existing shell', () => {
  const orderedComponents = [
    '<BuildHero', '<BuildHowItWorks', '<BuildWhatYouCanBuild', '<BuildEvidenceContext',
    '<BuildStageSelector', '<BuildFocusSection', '<Footer',
  ];
  let previousIndex = -1;
