...
test('journey upgrade prompts cover the founder journey triggers', () => {
  ...
  assert.doesNotMatch(icpSource, /trigger: "post_icp_nudge"/);
  assert.doesNotMatch(icpSource, /shouldShowPostIcpStarterNudge/);
  assert.match(icpSource, /first incomplete customer-interview task/);
