...
test('pmf funnel analytics events are wired on client actions', () => {
  ...
  const discoveryHook = read('src/hooks/useCustomerDiscovery.ts');
  ...
  assert.match(discoveryHook, /pmf_customer_discovery_started/);
  assert.match(discoveryHook, /pmf_customer_discovery_completed/);
});
