...
const displaySource = readFileSync(new URL('../src/components/CreditNavigationMenu.tsx', import.meta.url), 'utf8');
...
test('navigation credit balance stays inside the viewport with a thin scrollbar', () => {
  ...
  assert.match(displaySource, /min\(32rem, 70dvh, var\(--radix-dropdown-menu-content-available-height\)\)/);
