...
/*
 * Every other entry in this catalog promises "a saved X".
 *
 * That is the whole problem this one exists to fix. A saved ICP, a saved scope,
 * a saved GTM plan: each is a document the founder is the only person ever to
 * read, and each is a thing ChatGPT produces for twenty dollars a month. The
 * platform's one defensible property is that it hosts the artifact a founder
 * puts in front of buyers and therefore witnesses the response first-hand, and
 * no activation path led there.
 *
 * So the output here is a URL, and the last step is publishing rather than
 * saving. If that reads as a smaller promise than "a saved go-to-market plan",
 * it is the only one on this list a stranger can answer.
 */
export const ACTIVATION_CATALOG: Record<ActivationIntent, ActivationCatalogEntry> = {
  first_customer_sprint: { intent: 'first_customer_sprint', toolKey: 'gtm_strategist', label: 'Run First Customer Proof', description: 'Run a buyer-facing acquisition cycle with 10 qualified prospects, 10 founder-sent messages, and a decision.', output: 'Buyer evidence or an evidence-backed next decision', route: '/go-to-market?workspace=first-customer-proof&source=onboarding', estimatedMinutes: 5, artifactType: 'first_customer_proof', steps: ['Confirm the offer and buyer', 'Build the ten-prospect sample', 'Send, log evidence, and record the next decision'] },
  publish_proof: { intent: 'publish_proof', toolKey: 'demo_studio', label: 'Put it in front of a buyer', description: 'Publish a page a real prospect can open, react to, and leave their email on.', output: 'A live URL you can send to a stranger', route: '/demo-studio/classic?activationIntent=publish_proof', estimatedMinutes: 12, artifactType: 'published_proof', featureKey: 'demo_studio', steps: ['Describe what you are building', 'Add your proof and CTA', 'Publish and share the link'] },
...
const STAGE_FALLBACK: Record<FounderStageId, ActivationIntent> = { 1: 'run_icp', 2: 'build_demo', 3: 'start_validation', 4: 'first_customer_sprint', 5: 'first_customer_sprint', 6: 'log_traction', 7: 'analyze_pitch_deck' };
...
function blockerIntent(
  blocker: FounderBlocker,
  productStatus: ProductStatus,
  publishProofFirst = false,
): ActivationIntent {
  /*
   * A founder who says their blocker is demand cannot resolve it inside the
   * product. `start_validation` and `build_demo` both end in a saved document,
   * which is the founder asking themselves whether people want this. Publishing
   * is the only branch that puts the question to somebody else.
   *
   * Gated so the change is measurable: flag off restores the exact prior
   * routing, and the proof-loop funnel says whether this raised the number of
   * founders reaching a real stranger or just moved them to a step they abandon.
   */
  if (productStatus === 'live_product' && (blocker === 'demand_validation' || blocker === 'go_to_market')) return 'first_customer_sprint';
  if (publishProofFirst && blocker === 'demand_validation') return 'publish_proof';
  ...
  if (blocker === 'go_to_market') return productStatus === 'prototype_demo' ? 'plan_gtm' : 'first_customer_sprint';
...
export interface ActivationRecommendationInput {
  ...
  productStatus: ProductStatus;
  /**
   * Route early-stage founders to publishing rather than to another saved
   * document. Supplied by the caller from the `publish-proof-first` flag so this
   * function stays pure and node:test can load it, matching how
...
export function recommendActivation(input: ActivationRecommendationInput): ActivationRecommendation {
  ...
  const stageFallback = input.publishProofFirst && (input.assignedStage === 1 || input.assignedStage === 2)
    ? 'publish_proof'
    : STAGE_FALLBACK[input.assignedStage];
  const intent = available.has(byBlocker)
    ? byBlocker
    : available.has(stageFallback)
      ? stageFallback
