...
export async function streamPulseHome(input: {
  sessionId: string; turnId: string; message: string; context: Record<string, unknown>;
  ...
  const response = await fetch(`${import.meta.env.VITE_SUPABASE_URL}/functions/v1/chatbot-streaming`, {
    ...
    body: JSON.stringify({ surface: 'pulse_home', chatMode: 'pulse', sessionId: input.sessionId, turnId: input.turnId, message: input.message, businessContext: input.context }),
  });
  if (!response.ok || !response.body) throw new Error(response.status === 401 ? 'Please sign in again to continue.' : 'Pulse could not connect. Your message is ready to retry.');
