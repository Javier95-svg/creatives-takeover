...
export default function GTMExecutionOS({ plan, planId, mode, onUpdatePlan }: GTMExecutionOSProps) {
      ...
      <div className="grid gap-5 lg:grid-cols-[1.1fr_0.9fr]">
        <Card className="border-2 border-primary/20">
          ...
          <CardContent className="space-y-4">
            ...
            <div className="rounded-xl border border-border/60 p-3 text-xs text-muted-foreground"><p className="font-medium text-foreground">Auditable inputs</p><ul className="mt-2 space-y-1">{health.calculation?.map((item) => <li key={item}>• {item}</li>)}</ul>{typeof health.attributedPipelineValue === 'number' ? <p className="mt-2 font-medium text-foreground">Attributed active pipeline: ${health.attributedPipelineValue.toLocaleString()}</p> : null}</div>
  ...
  if (mode === 'competitors') {
    return (
      <Card><CardHeader><div className="flex items-center gap-2"><Swords className="h-5 w-5 text-primary" /><CardTitle>Evidence-backed battlecards</CardTitle></div><p className="text-sm text-muted-foreground">Unverified fields are labeled instead of presented as competitor facts.</p></CardHeader><CardContent className="grid gap-4 md:grid-cols-2">{competitors.length > 0 ? competitors.map((competitor) => <div key={competitor.id} className="space-y-3 rounded-2xl border border-border/60 p-4"><div className="flex items-start justify-between gap-2"><div><p className="font-semibold">{competitor.name}</p><p className="text-xs text-muted-foreground">{competitor.category} · {competitor.pricing || 'pricing unverified'}</p></div><Badge variant="outline">{competitor.lastVerifiedAt ? `verified ${competitor.lastVerifiedAt.slice(0, 10)}` : 'unverified'}</Badge></div><p className="text-sm">{competitor.positioning}</p>{competitor.acquisitionChannels?.length ? <p className="text-xs text-muted-foreground">Visible channels: {competitor.acquisitionChannels.join(' · ')}</p> : null}<div className="grid grid-cols-2 gap-3 text-xs"><div><p className="font-medium text-success">Strengths</p>{competitor.strengths.map((item) => <p key={item} className="mt-1 text-muted-foreground">• {item}</p>)}</div><div><p className="font-medium text-primary">Your opening</p>{competitor.gaps.map((item) => <p key={item} className="mt-1 text-muted-foreground">• {item}</p>)}</div></div>{competitor.proofPoints?.length ? <div className="rounded-xl bg-muted/30 p-3 text-xs"><p className="font-medium">Observed proof</p>{competitor.proofPoints.map((item) => <p key={item} className="mt-1 text-muted-foreground">• {item}</p>)}</div> : null}<div className="rounded-xl border border-primary/20 bg-primary/5 p-3 text-xs"><p className="font-medium">Likely objection</p><p className="mt-1 text-muted-foreground">{competitor.likelyObjection}</p><p className="mt-2 font-medium">Recommended response</p><p className="mt-1 text-muted-foreground">{competitor.recommendedResponse}</p></div>{competitor.sourceUrls.length ? <div className="flex flex-wrap gap-2">{competitor.sourceUrls.map((url) => <a key={url} href={url} target="_blank" rel="noreferrer" className="text-xs text-primary underline">Source</a>)}</div> : null}</div>) : <p className="text-sm text-muted-foreground">No named alternatives were found. Add competitors in Diagnose and regenerate when the market is clearer.</p>}</CardContent></Card>
    );
  }
  ...
  return (
    <div className="space-y-6">
      ...
      <Card><CardHeader><div className="flex items-center gap-2"><FileText className="h-5 w-5 text-primary" /><CardTitle>Channel-native asset packs</CardTitle></div><p className="text-sm text-muted-foreground">Each active play receives measurement, message, conversion, and learning assets. Nothing is sent or published until you approve it.</p></CardHeader><CardContent className="grid gap-4 lg:grid-cols-2">{assets.map((asset) => <AssetEditor key={asset.id} asset={asset} onSave={updateAsset} />)}</CardContent></Card>
    </div>
  );
}
