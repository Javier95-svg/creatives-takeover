...
interface Invitation { id: string; email: string; user_type: string; expires_at: string; revoked_at: string | null; claimed_by: string | null }
...
export function AccountInvitations() {
  ...
  const manage = useMutation({ mutationFn: async (input: { email: string; category: string; revoke?: boolean }) => {
    const { error } = await supabase.rpc('manage_account_invitation' as never, {
      p_email: input.email.trim(), p_user_type: input.category, p_revoke: input.revoke ?? false,
    } as never);
    if (error) throw new Error(error.message);
  }, onSuccess: (_data, input) => {
    toast.success(input.revoke ? 'Invitation revoked.' : 'Invitation saved for 30 days. No email has been sent.');
  ...
  return <section className="mb-8 rounded-xl border p-5" aria-labelledby="invitations-heading">
    ...
    <p className="my-2 text-sm text-muted-foreground">Invite a person using their sign-in email. They must verify that email, answer the onboarding quiz, and receive your approval. Existing approved accounts keep their access. Saving an invitation does not send an email; share your usual onboarding link with the person.</p>
    ...
    <ul className="space-y-2">{(invitations.data ?? []).map(invitation => <li key={invitation.id} className="flex flex-wrap items-center gap-3 border-t pt-2 text-sm">
      <span>{invitation.email} · {invitation.user_type} · {invitation.revoked_at ? 'Revoked' : new Date(invitation.expires_at) <= new Date() ? 'Expired' : `Expires ${new Date(invitation.expires_at).toLocaleDateString()}`}{invitation.claimed_by ? ' · Request submitted' : ''}</span>
