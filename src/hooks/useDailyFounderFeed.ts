...
export function useDailyFounderFeed() {
  ...
  const cards = useMemo<FounderFeedCard[]>(() => {
    ...
    const focusCard: FounderFeedCard = {
      ...
      actionLabel: primaryStageTool ? `Open ${primaryStageTool.name}` : 'Open Tasks',
      actionRoute: primaryStageTool?.route ?? '/dashboard/tasks',
      priority: 82,
      freshnessDate: bizMapProgress.progress?.updated_at ?? now,
      sourceLabel: 'Stage progress',
      stage,
    ...
    const recommendationCards = (personalizedData?.recommendations ?? [])
      ...
      .map<FounderFeedCard>((item) => ({
        ...
        actionRoute: item.action_url || '/dashboard/tasks',
    ...
    sourceData?.mentors.forEach((mentor) => {
      platformCards.push({
        ...
        actionRoute: `/mentorship/mentors/${mentor.id}`,
        priority: 64,
    ...
    [...(sourceData?.angels ?? []), ...(sourceData?.investors ?? [])].slice(0, 1).forEach((investor) => {
      platformCards.push({
        ...
        actionRoute: 'sectors' in investor ? '/investors' : `/insighta/vc/${investor.slug}`,
        priority: 62,
