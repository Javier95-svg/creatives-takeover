...
export default function FirstCustomerSprintPage() {
  ...
  const isPaidConciergeSprint = sprint?.offer_version === 'concierge_299';
  ...
  const [publishedDemos, setPublishedDemos] = useState<Array<{ id: string; name: string }>>([]);
  const [selectedDemoId, setSelectedDemoId] = useState('');
  const [demoEvidence, setDemoEvidence] = useState<FirstCustomerDemoEvidence | null>(null);
  ...
  useEffect(() => {
    if (!user) return;
    void (supabase as any).from('demo_studio_projects').select('id,name').eq('owner_id', user.id).eq('launch_published', true).order('updated_at', { ascending: false }).then(({ data }: { data?: Array<{ id: string; name: string }> }) => setPublishedDemos(data ?? []));
  }, [user]);

  useEffect(() => {
    if (!sprint?.id) { setDemoEvidence(null); return; }
    setSelectedDemoId(sprint.source_demo_project_id ?? '');
    void sprintApi.getDemoEvidence(sprint.id).then(setDemoEvidence).catch(() => setDemoEvidence(null));
  }, [sprint?.id, sprint?.source_demo_project_id]);
  ...
  const attachDemo = () => {
    if (!sprint || !selectedDemoId) return;
    void safely(async () => {
      await sprintApi.attachDemo(sprint.id, selectedDemoId);
      const summary = await sprintApi.getDemoEvidence(sprint.id);
      setDemoEvidence(summary);
      trackFirstCustomerSprint('first_customer_sprint_demo_attached', { sprint_id: sprint.id, demo_project_id: selectedDemoId });
    }, 'Published demo attached as the sprint proof artifact.');
  };
  ...
  if (!sprint) return (
    <><Helmet><title>Start your First Customer Sprint</title><meta name="robots" content="noindex,nofollow" /></Helmet><Navigation /><main className="container mx-auto max-w-4xl px-4 py-16 pt-28"><div className="mb-8"><Badge>Invite accepted</Badge><h1 className="mt-3 text-4xl font-bold">Reach three qualified buyer conversations in 30 days</h1><p className="mt-3 text-muted-foreground">You perform the outreach. The product keeps the evidence, messaging, and one mentor checkpoint focused; it does not guarantee a sale.</p></div><Card><CardHeader><CardTitle>Define the sprint</CardTitle><CardDescription>All fields stay editable. A beta-cohort invitation can override the normal B2B SaaS/service eligibility rule.</CardDescription></CardHeader><CardContent className="space-y-4">
      ...
      <div className="grid gap-4 sm:grid-cols-2"><label className="space-y-1 text-sm"><span>Proof URL (optional)</span><Input type="url" value={intake.proofUrl} onChange={(e) => setIntake((v) => ({ ...v, proofUrl: e.target.value }))} /></label><label className="space-y-1 text-sm"><span>Proof description (optional)</span><Input value={intake.proofDescription} onChange={(e) => setIntake((v) => ({ ...v, proofDescription: e.target.value }))} placeholder="A concept demo can be prepared during the sprint" /></label></div>
      <div className="grid gap-4 sm:grid-cols-2"><label className="space-y-1 text-sm"><span>Estimated customer value (optional hypothesis)</span><Input type="number" min="0" value={intake.estimatedCustomerValueUsd} onChange={(e) => setIntake((v) => ({ ...v, estimatedCustomerValueUsd: e.target.value }))} /></label><label className="space-y-1 text-sm"><span>Weekly capacity (hours, minimum 2) *</span><Input type="number" min="2" value={intake.weeklyCapacityHours} onChange={(e) => setIntake((v) => ({ ...v, weeklyCapacityHours: e.target.value }))} /></label></div>
  ...
  const mentorUnlocked = evidence.attachedProspects >= 10 && Boolean(sprint.selected_message_variant);
  ...
  return (
    <><Helmet><title>First Customer Sprint | Creatives Takeover</title><meta name="robots" content="noindex,nofollow" /></Helmet><Navigation /><main className="container mx-auto max-w-6xl space-y-8 px-4 py-16 pt-28">
      ...
      <Card><CardHeader><CardTitle>Attach a published proof demo</CardTitle><CardDescription>Demo completions, CTA clicks, leads, and signups appear as verified behavioral context. Raw views never satisfy a sprint outcome, and demo behavior never replaces qualified conversations.</CardDescription></CardHeader><CardContent className="space-y-4"><div className="flex flex-col gap-2 sm:flex-row"><select className="h-10 flex-1 rounded-md border bg-background px-3 text-sm" value={selectedDemoId} onChange={(event) => setSelectedDemoId(event.target.value)}><option value="">Select a published Demo Studio project…</option>{publishedDemos.map((demo) => <option key={demo.id} value={demo.id}>{demo.name}</option>)}</select><Button variant="outline" disabled={!selectedDemoId || sprintApi.isSaving} onClick={attachDemo}>Attach demo</Button></div>{demoEvidence ? <div className="grid grid-cols-2 gap-3 text-center sm:grid-cols-4">{[['Completions', demoEvidence.completions], ['CTA clicks', demoEvidence.ctaClicks], ['Leads', demoEvidence.leads], ['Signups', demoEvidence.signups]].map(([label, value]) => <div key={String(label)} className="rounded-lg border p-3"><p className="text-2xl font-bold">{value}</p><p className="text-xs text-muted-foreground">{label} · verified</p></div>)}</div> : null}</CardContent></Card>
      ...
      <Card id="mentor-checkpoint"><CardHeader><SectionHeading number={4} title="Mentor checkpoint" description="At 10 prospects and a selected message, request an admin-coordinated checkpoint with one recommended GTM mentor. No credits are deducted." state={sprint.mentor_checkpoint_completed_at ? 'verified with recommendation' : sprint.checkpoint_status} /></CardHeader><CardContent className="space-y-5">
