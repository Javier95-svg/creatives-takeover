...
import type { FirstCustomerDecision, FirstCustomerMessageVariant } from '@/types/firstCustomerSprint';
...
export default function FirstCustomerSprintPage() {
  ...
  const milestonesRef = useRef({ outreach: false, conversation: false });
  ...
  }, [evidence.contactedProspects, evidence.conversations, sprint]);
  ...
  const recordCheckpoint = () => {
    ...
    void safely(() => sprintApi.update(sprint.id, {
      mentorCheckpointCompleted: true, mentorRecommendationSummary: decisionNotes,
      finalDecision: decision, finalNotes: decisionNotes,
    }), 'Mentor checkpoint recorded.');
  };
  ...
  if (!sprintApi.enabled || !sprintApi.enrolled) return (
    <><Helmet><title>First Customer Sprint | Creatives Takeover</title></Helmet><Navigation /><main className="container mx-auto min-h-screen max-w-3xl px-4 py-24"><Card><CardHeader><Badge className="mb-3 w-fit" variant="outline">Invite-only pilot</Badge><CardTitle>{sprintApi.enabled ? 'This sprint is currently invite-only' : 'The First Customer Sprint is not open yet'}</CardTitle><CardDescription>{sprintApi.enabled ? 'We are starting with a small founder cohort so we can learn before opening access more broadly.' : 'The pilot is behind a release switch while we finish deployment checks.'}</CardDescription></CardHeader><CardContent><Button asChild variant="outline"><Link to="/dashboard">Return to dashboard</Link></Button></CardContent></Card></main><Footer /></>
  );

  if (!sprint) return (
    <><Helmet><title>Start your First Customer Sprint</title><meta name="robots" content="noindex,nofollow" /></Helmet><Navigation /><main className="container mx-auto max-w-4xl px-4 py-16 pt-28"><div className="mb-8"><Badge>Invite accepted</Badge><h1 className="mt-3 text-4xl font-bold">Win the next customer in 30 days</h1><p className="mt-3 text-muted-foreground">You do the customer work. The product keeps the evidence, messaging, and mentor checkpoint focused.</p></div><Card><CardHeader><CardTitle>Define the sprint</CardTitle><CardDescription>All fields stay editable. A beta-cohort invitation can override the normal B2B SaaS/service eligibility rule.</CardDescription></CardHeader><CardContent className="space-y-4">
  ...
  const completionEligible = Boolean(snapshot?.canComplete) || canCompleteFirstCustomerSprint(evidence, decision, decisionNotes);
  ...
  return (
    <><Helmet><title>First Customer Sprint | Creatives Takeover</title><meta name="robots" content="noindex,nofollow" /></Helmet><Navigation /><main className="container mx-auto max-w-6xl space-y-8 px-4 py-16 pt-28">
      ...
      <Card id="mentor-checkpoint"><CardHeader><SectionHeading number={4} title="Mentor checkpoint" description="At 10 prospects and a selected message, bring this one-page brief to one of the two recommended GTM mentors. The existing 10-credit call policy applies." state={sprint.mentor_checkpoint_completed_at ? 'recorded' : mentorUnlocked ? 'recommended now' : 'prepare while targeting'} /></CardHeader><CardContent className="space-y-5">
        ...
        <div className="grid gap-4 md:grid-cols-2">{mentors.loading ? <Loader2 className="h-6 w-6 animate-spin" /> : mentors.recommendations.map(({ mentor, reason }) => <div key={mentor.id} className="rounded-xl border p-4"><div className="flex items-start gap-3"><Users className="mt-1 h-5 w-5 text-primary" /><div><p className="font-semibold">{mentor.name}</p><p className="mt-1 text-sm text-muted-foreground">{reason}</p>{mentorUnlocked ? <Button asChild className="mt-3" size="sm"><Link to={`/mentorship/book/${mentor.id}?sprint=${sprint.id}`}>Book for 10 credits <ArrowRight className="ml-2 h-4 w-4" /></Link></Button> : <Button className="mt-3" size="sm" disabled>Attach 10 prospects and select a message</Button>}</div></div></div>)}</div>
        <div className="rounded-xl border border-primary/20 bg-primary/5 p-4"><h3 className="font-semibold">After the call</h3><p className="mt-1 text-sm text-muted-foreground">Record what changed; the mentor does not need platform access.</p><div className="mt-3 grid gap-3 sm:grid-cols-[240px_minmax(0,1fr)]"><select className="h-10 rounded-md border bg-background px-3 text-sm" value={decision} onChange={(e) => setDecision(e.target.value as FirstCustomerDecision)}>{DECISIONS.map((item) => <option key={item.value} value={item.value}>{item.label}</option>)}</select><Textarea value={decisionNotes} onChange={(e) => setDecisionNotes(e.target.value)} placeholder="What did the mentor change in your segment, offer, message, channel, or decision?" /></div><Button className="mt-3" variant="outline" onClick={recordCheckpoint}>Record checkpoint decision</Button></div>
      </CardContent></Card>
