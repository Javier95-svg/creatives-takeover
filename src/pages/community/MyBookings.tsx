  ...
  const renderCard = (booking: DiscoveryCallBookingItem) => {
    ...
    return <Card key={booking.id} id={`call-${booking.id}`}><CardContent className="p-6">
      ...
      <div className="mt-3 flex flex-wrap gap-4 text-sm text-muted-foreground">
        ...
        {booking.reservation?.status === 'pending' && booking.reservation.held_amount > 0 && <span>{booking.reservation.held_amount} credits held</span>}
      ...
      <div className="mt-4 flex flex-wrap gap-2">
        ...
        {pending && <Button size="sm" variant="destructive" disabled={busyId === booking.id} onClick={() => window.confirm('Withdraw this request?') && void run(booking.id, () => withdrawDiscoveryCallRequest(booking.id))}>Withdraw request</Button>}
        {creatingMeeting && <Button size="sm" variant="destructive" disabled={busyId === booking.id} onClick={() => window.confirm('Cancel this reservation?') && void run(booking.id, () => cancelDiscoveryCall(booking.id, 'Founder cancelled while meeting link was being created'))}>Cancel reservation</Button>}
