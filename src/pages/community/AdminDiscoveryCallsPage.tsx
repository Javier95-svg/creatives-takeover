...
type OutcomeAction = 'completed' | 'founder_no_show' | 'mentor_no_show' | 'record_agreement';
type OutcomeDraft = { callId: string; action: OutcomeAction; agreement: string; summary: string; nextStep: string; evidenceSource: string };
...
export default function AdminDiscoveryCallsPage() {
  ...
  const saveOutcome = async () => {
    ...
    if (outcomeDraft.summary.trim().length < 10 || !outcomeDraft.evidenceSource || (['completed', 'record_agreement'].includes(outcomeDraft.action) && !outcomeDraft.agreement)) {
      toast.error('Enter a summary, evidence source, and agreement result for an attended call.');
      return;
    }
    setBusy(outcomeDraft.callId);
    try {
      const response = await adminOverrideDiscoveryCall({
        callId: outcomeDraft.callId, overrideAction: outcomeDraft.action,
        reason: outcomeDraft.summary.trim(), agreement: ['completed', 'record_agreement'].includes(outcomeDraft.action) ? outcomeDraft.agreement : null,
        nextStep: outcomeDraft.nextStep.trim(), evidenceSource: outcomeDraft.evidenceSource,
      });
      if (!response.success) throw new Error(response.error || response.errorCode || 'Unable to record outcome.');
  ...
  return <><Helmet><title>Discovery Calls Admin | Creatives Takeover</title><meta name="robots" content="noindex,nofollow" /></Helmet><Navigation /><main className="container mx-auto min-h-screen px-4 pb-16 pt-header-offset"><div className="mb-6 flex flex-wrap items-center justify-between gap-3"><div><h1 className="text-3xl font-bold">Discovery Calls</h1><p className="text-muted-foreground">Workflow, credits, Google Meet creation, audit, and email delivery.</p></div><Button variant="outline" onClick={() => void load()} disabled={loading}><RefreshCw className="mr-2 h-4 w-4" />Refresh</Button></div>
    ...
    {loading ? <Card><CardContent className="flex justify-center p-10"><Loader2 className="mr-2 h-5 w-5 animate-spin" />Loading…</CardContent></Card> : <div className="space-y-5">{calls.map((call) => {
      ...
      return <Card key={callId}><CardHeader><CardTitle className="flex flex-wrap items-center justify-between gap-2"><span>{value(call, 'mentor_name_snapshot')}</span><Badge variant="outline">{value(call, 'status')}</Badge></CardTitle></CardHeader><CardContent className="space-y-4"><div className="grid gap-2 text-sm md:grid-cols-2"><p><strong>Call:</strong> {callId}</p><p><strong>Founder:</strong> {value(call, 'founder_email_snapshot')}</p><p><strong>Mentor:</strong> {value(call, 'mentor_contact_email_snapshot')}</p><p><strong>Scheduled:</strong> {value(call, 'scheduled_for') ? new Date(value(call, 'scheduled_for')).toLocaleString() : 'Not confirmed'}</p><p><strong>Deadline:</strong> {value(call, 'response_due_at') ? new Date(value(call, 'response_due_at')).toLocaleString() : '—'}</p><p><strong>Credit hold:</strong> {reservation ? `${value(reservation, 'status')} (${value(reservation, 'held_amount')} credits)` : 'Legacy/no hold'}</p><p><strong>Meeting:</strong> {value(call, 'meeting_url') || value(call, 'meeting_creation_status') || 'Not created'}</p><p><strong>Google event:</strong> {value(call, 'external_calendar_event_id') || 'Not created'}</p></div>
        ...
        {outcome && <div className="rounded border p-3 text-sm"><p><strong>Agreement:</strong> {value(outcome, 'agreement') || 'Not applicable'}</p><p><strong>Evidence:</strong> {value(outcome, 'evidence_source')}</p><p><strong>Summary:</strong> {value(outcome, 'summary')}</p><p><strong>Next step:</strong> {value(outcome, 'next_step') || 'None recorded'}</p></div>}
        <div className="flex flex-wrap gap-2"><Button size="sm" variant="outline" disabled={busy === callId} onClick={() => void correctBooking(call)}>Correct time/logistics</Button>{(value(call, 'status') === 'awaiting_outcome' || (value(call, 'status') === 'scheduled' && value(call, 'scheduled_for') && Date.parse(value(call, 'scheduled_for')) + Number(call.duration_minutes ?? 30) * 60_000 <= Date.now())) && (<><Button size="sm" disabled={busy === callId} onClick={() => setOutcomeDraft({ callId, action: 'completed', agreement: '', summary: '', nextStep: '', evidenceSource: '' })}>Record completed call</Button><Button size="sm" variant="outline" disabled={busy === callId} onClick={() => setOutcomeDraft({ callId, action: 'founder_no_show', agreement: '', summary: '', nextStep: '', evidenceSource: '' })}>Founder no-show</Button><Button size="sm" variant="outline" disabled={busy === callId} onClick={() => setOutcomeDraft({ callId, action: 'mentor_no_show', agreement: '', summary: '', nextStep: '', evidenceSource: '' })}>Mentor no-show + refund</Button></>)}{value(call, 'status') === 'completed' && <Button size="sm" disabled={busy === callId} onClick={() => setOutcomeDraft({ callId, action: 'record_agreement', agreement: value(outcome ?? {}, 'agreement'), summary: value(outcome ?? {}, 'summary'), nextStep: value(outcome ?? {}, 'next_step'), evidenceSource: value(outcome ?? {}, 'evidence_source') })}>{outcome ? 'Update agreement' : 'Record agreement'}</Button>}<Button size="sm" variant="destructive" disabled={busy === callId} onClick={() => void override(callId, 'cancel_refund')}>Cancel + refund/release hold</Button><Button size="sm" variant="destructive" disabled={busy === callId} onClick={() => void override(callId, 'cancel_no_refund')}>Cancel, no refund</Button></div>
        {outcomeDraft?.callId === callId && <div className="space-y-3 rounded border p-4"><h3 className="font-semibold">Record {outcomeDraft.action.replaceAll('_', ' ')}</h3><label className="block text-sm">How was this verified?<select className="mt-1 w-full rounded border bg-background p-2" value={outcomeDraft.evidenceSource} onChange={(event) => setOutcomeDraft({ ...outcomeDraft, evidenceSource: event.target.value })}><option value="">Choose source</option><option value="founder">Founder</option><option value="mentor">Mentor</option><option value="both">Both participants</option><option value="admin">Admin observation</option></select></label>{['completed', 'record_agreement'].includes(outcomeDraft.action) && <label className="block text-sm">Agreed to work together?<select className="mt-1 w-full rounded border bg-background p-2" value={outcomeDraft.agreement} onChange={(event) => setOutcomeDraft({ ...outcomeDraft, agreement: event.target.value })}><option value="">Choose result</option><option value="yes">Yes</option><option value="no">No</option><option value="undecided">Undecided</option><option value="unknown">Unknown</option></select></label>}<label className="block text-sm">Outcome summary<Textarea className="mt-1" maxLength={2000} value={outcomeDraft.summary} onChange={(event) => setOutcomeDraft({ ...outcomeDraft, summary: event.target.value })} placeholder="What happened? Include how attendance was confirmed." /></label><label className="block text-sm">Next step<Input className="mt-1" maxLength={2000} value={outcomeDraft.nextStep} onChange={(event) => setOutcomeDraft({ ...outcomeDraft, nextStep: event.target.value })} placeholder="Optional follow-up or commitment" /></label><div className="flex gap-2"><Button size="sm" disabled={busy === callId} onClick={() => void saveOutcome()}>Save outcome</Button><Button size="sm" variant="outline" onClick={() => setOutcomeDraft(null)}>Cancel</Button></div></div>}
      </CardContent></Card>;
